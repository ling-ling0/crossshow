...
- 待办开关（2026-09-16 新增）：待办是没有固定时间但需要做的事情，作为纯记录不关联任何日期；仅创建时可选，不支持重复，日视图中显示在待办栏。
- 全天开关。
- 普通事件：开始日期时间、结束日期时间。
...
日视图（2026-09-16 新增）在时间区域上方提供待办栏，集中显示全部待办（没有固定时间、不关联日期的记录，跨日期持续可见，做完删除即可）；待办不进入全天区域与时间网格，周视图不按日展示待办。范围查询始终包含待办。
...
| all_day | 是否全天 |
| is_todo | 是否待办（没有固定时间的记录）；与全天互斥，不携带任何时间/日期字段，范围查询始终返回 |
